const displayvideos = videos => {
    ...
    videos.data.forEach(video => {
        ...
        <h2 class="card-title2 font-normal text-slate-700 text-2xl">${video?.authors[0]?.profile_name}</h2>
         <span class="pt-1"> ${video?.authors[0]?.verified ? '<i class="fa-solid fa-certificate text-blue-600"></i>' : ' '}
        </span>
        </div>
...
const displaybutton = button => {
    ...
    button.forEach(menu => {
        ...
        menuBar.innerHTML = `
        <div onclick="handleChatagory(${menu?.category_id})" class=" lg:pt-3 md:pt-0 pt-2 text-center block  h-[35px] lg:h-[55px] md:h[40px] rounded lg:w-[150px] w-14 md:w-20 lg:text-xl text-sm md:text-lg font-semibold text-black  bg-gray-100">${menu?.category}</div>
        `;
...
const notFound = (data) => {
    ...
    const notTrue = !data.status ? `<div class="flex flex-col lg:mb-0 md:mb-8 mb-4 mt-10 md:mt-36 lg:mt-44 items-center gap-4">
